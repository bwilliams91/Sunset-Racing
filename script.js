...
function calcMovement() {
    ...
    if ($.state.speed) {
        if ($.state.keypress.left) {
            $.state.xpos += (Math.abs($.state.turn) + 7 + ($.state.speed > $.state.car.maxSpeed / 4 ? ($.state.car.maxSpeed - ($.state.speed / 2)) : $.state.speed)) * .2;
            $.state.turn -= 1;
        }
...
function drawBg() {
    console.log('ping drawBg')

    // Define the sky gradient
const skyGradient = $.ctx.createLinearGradient(0, 0, 0, $.canvas.height);
skyGradient.addColorStop(0, '#ff6e40'); // color at the top (sunset color)
skyGradient.addColorStop(1, '#1a237e'); // color at the bottom (darker blue)
$.colors.sky = skyGradient; // Assign the gradient to $.colors.sky
...
function drawGround(ctx, offset, lightColor, darkColor, width) {
    ...
    while (pos <= $.canvas.height) {
        console.log('pos is less than or equal to canvas height')
        stepSize = norm(pos, $.settings.skySize, $.canvas.height) * $.settings.ground.max;
        if (stepSize < $.settings.ground.min) {
            console.log('stepsize is less than ground min', $.settings.ground.min)
            stepSize = $.settings.ground.min;
        }

        if (drawDark) {

            console.log('drawDark')
